...
- 无实时连接时工作台才会显示明确的离线缓存/演示标记；实时靶点卡、评分和 Decision Memo 必须以连接器状态和原始来源为准。
...
- 连续追问以 `session_id` 为边界，历史消息、当前靶点卡和 DeepSeek 上下文一起恢复；刷新研究只递增卡片版本，不新建会话。
...
- 数据库模式不再注入 ROR1 等预置演示会话；真实工作台只显示用户实际创建并完成检索的记录，ROR1 fixture 仅保留给离线 API 测试。
