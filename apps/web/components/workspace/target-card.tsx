    ...
    <article className="target-card">
      ...
      <div className="target-card-body">
        ...
        <div className="target-sections">
          ...
          <EvidenceSection number="04" title="代表药物" summary="代表项目用于识别成药路径和开发阶段，不能单独替代疗效判断。" evidenceCount={Math.min(card.drugs.length, card.validation.length)} onEvidence={openFirstEvidence}>
          ...
          <EvidenceSection number="05" title="临床进展" summary="登记信息用于确认项目和阶段，不能单独替代疗效判断。" evidenceCount={Math.min(card.trials.length, card.validation.length)} onEvidence={openFirstEvidence}>
            {card.trials.length > 0 ? <div className="trial-strip">{card.trials.map((trial) => <button key={trial.identifier} className="trial-chip" onClick={() => { const evidence = evidenceById.get(trial.sourceId) ?? firstEvidence; if (evidence) onEvidence(evidence); }}><span>{trial.identifier}</span><small>{trial.phase} · {trial.status}</small><ExternalLink size={13} /></button>)}</div> : <p className="empty-inline-state">当前来源未返回可归一化的临床登记，需扩大检索范围后复核。</p>}
          </EvidenceSection>

          <EvidenceSection number="06" title="失败风险" summary="优先展示可能导致项目失败的证据边界，不把来源命中直接当成安全或疗效结论。" evidenceCount={card.risks.length} onEvidence={openLastEvidence}>
